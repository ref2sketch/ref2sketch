...
class Train:
    ...
    def train(self):
        ...
        zeros = np.zeros((1,128,128,128))
        zeros = torch.from_numpy(zeros)
        zeros = zeros.to(device)

        dataset_train = Dataset(dir_data_train, direction=self.direction, data_type=self.data_type, nch=self.nch_in)
        dataset_val = Dataset(dir_data_val, direction=self.direction, data_type=self.data_type, nch=self.nch_in)
        ...
        for epoch in range(st_epoch + 1, num_epoch + 1):
            ...
            for i, data in enumerate(loader_train, 1):
                ...
                # forward netG
                output,spatial= netG(input,style)
                ...
                loss_G_l1_1 = fn_L1(spatial, zeros)
                loss_G = (wgt_l1 * loss_G_l1) + (wgt_gan * loss_G_gan) + per_loss_rec  + (loss_G_l1_1*sparse_loss)
                loss_G.backward()
                optimG.step()
                ...
                loss_G_l1_1_train += [loss_G_l1_1.item()]
                loss_D_fake_train += [loss_D_fake.item()]
                loss_D_real_train += [loss_D_real.item()]

                print('TRAIN: EPOCH %d: BATCH %04d/%04d: '
                      'GEN L1: %.4f GEN GAN: %.4f GEN VGG16: %.4f  GEN L1_1: %.4f  DISC FAKE: %.4f DISC REAL: %.4f'
                      % (epoch, i, num_batch_train,
                         mean(loss_G_l1_train), mean(loss_G_gan_train), mean(loss_G_vgg16),mean(loss_G_l1_1_train), mean(loss_D_fake_train), mean(loss_D_real_train)))
